# coding=utf-8
# Copyright 2018 The Google AI Language Team Authors. Modified By Zijing Zhang
#
    ...
    def __init__(self, json_example):
        ...
        if FLAGS.dataset == 'train':
            ...
            if len(json_example['annotations']) != 5:
                raise ValueError(
                    'Dev set json_examples should have five annotations.')
        ...
        if self.has_long_answer:
            ...
            # print("Passage: "+self.long_answer_text)
            passage = self.long_answer_text
        ...
        if self.has_short_answer:
            ...
            self.short_answers_text = self.short_answers_texts[np.argmax(
                short_answers_counts)]
            answer = self.short_answers_text
...
class NqServer(object):
    ...
    def serve(self):
        ...
        server = wsgiref.simple_server.make_server(
            '', FLAGS.port, self.application)
        server.serve_forever()


def save(passage, answer, question):
    """Save dataset for generation"""
    datum = {}
    datum['passage'] = passage
    datum['answer'] = answer
